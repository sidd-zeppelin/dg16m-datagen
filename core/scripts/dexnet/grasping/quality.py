...
class PointGraspMetrics3D:
    ...
    def force_closure_original(c1, c2, friction_coef, use_abs_value=True):
        """" Checks force closure using the antipodality trick.
    ...
    def Dual_force_closure_batch(points, normals, params=None):
        ...
        result1, result2 = fc_loss.fc_loss(points, normals) # result1: 8a, result2: 8b
        result1 = result1.cpu().numpy()
        result2 = result2.cpu().numpy()

        result1 = np.where(result1 < 1e-3, result1, 0) # 0 is not qualified
        # print("Result2")
        # np.save('./result2.npy', result2)
        # np.save('./result1.npy', result1)
        # exit()
        print("Before:", result2.shape, np.min(result2), np.max(result2), np.mean(result2))
        result2 = np.where(result2 < 0.05, result2, 0) # 0 is not qualified
        print("After:", result2.shape, np.min(result2), np.max(result2), np.mean(result2))
        print(f"R1: {np.sum(result1 != 0)}, R2: {np.sum(result2 != 0)}, Intersection:{np.sum((result1 * result2) != 0)}")
